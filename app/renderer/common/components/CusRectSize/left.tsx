...
class LeftComponent extends React.Component<IProps, IState>{
    ...
    constructor(props: IProps) {
        ...
        this.isTransition = false; // 只有点击才加动画
    }

    componentDidMount() {
        if(this.boxRef.current && this.boxRef.current.children && this.boxRef.current.children.length > 0 && this.boxRef.current.children[0].clientWidth){
            this.setState({
                width: this.boxRef.current.children[0].clientWidth
            })
        }
    }
            ...
            <div key={key}>
               <div ref={this.boxRef} className="left-box" style={{width, ...style, left: showMenu ? style?.left : -width}}>
                ...
                <div className="react-menu"
                    ...
                    <div className={classnames('rect-icon', {
